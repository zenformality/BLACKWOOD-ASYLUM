...
def create_level():
    ...
    The map is a MAP_H × MAP_W list of tiles (ints or short strings).
    ...
    _add_corridor(m, 5, 11, 5, 11)
    _add_corridor(m, 5, 10, 5, 10)
    ...
    # ── Notes ────────────────────────────────────────────────────────────────
    m[14][4]  = 'n1'   # Entrance
    m[14][16] = 'n2'   # Main Hall
    m[5][17]  = 'n3'   # North Wing
    m[15][36] = 'n4'   # Far East
    m[24][24] = 'n5'   # Basement
...
def try_interact(player, tile_map, story_notes):
    ...
    for dy in range(-2, 3):
        for dx in range(-2, 3):
            ...
            # Notes
            if isinstance(tile, str) and tile.startswith('n'):
                note_id = tile[1:]
                if note_id in story_notes:
                    player.notes.add(note_id)
                    return "note", story_notes[note_id]
